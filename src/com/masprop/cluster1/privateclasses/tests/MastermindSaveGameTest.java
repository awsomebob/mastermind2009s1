...
public class MastermindSaveGameTest {
    ...
    public void run() {
         ...
         */
        //Used for writing a new file for our game (meaning that we will load in an empty file)
        ...
        * matrix_y  : which line
        * matrix_x :  which row
        * value : which colorcode (currently any int value is ok for testing)
        */
        int matrix_y = 10;
        int matrix_x = 3;
        int value = 4;
        ...
        MastermindGameFileParser parser = new MastermindGameFileParser(writenewfile);

        //read the file - USED FOR LOADING A GAME
        //also sets the status to null since we want to load our new status into that
        ...
        if(mastermind != null) {
            ...
            //these values are not changable since our gui will only have this grid
            int width = 4;
            int height = 8;
         ...
         */
        mastermind.getMastermindStatus().getMatrixMastermind().setCellValue(matrix_y, matrix_x, value);
